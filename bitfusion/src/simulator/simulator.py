...
class Simulator(object):
    ...
    def get_energy_cost(self):
        ...
        if len(core_data) == 0:
            ...
            core_data = lookup_pandas_dataframe(core_synth_data, lookup_dict)
            assert len(core_data) == 1
            core_area = float(core_data['Area (um^2)'].iloc[0]) * 1.e-6 * (N * M) / 16.
            core_dyn_power = float(core_data['Dynamic Power (nW)'].iloc[0]) * (N * M) / 16.
            core_dyn_energy = core_dyn_power / float(core_data['Frequency'].iloc[0])
            core_leak_power = float(core_data['Leakage Power (nW)'].iloc[0]) * (N * M) / 16.
